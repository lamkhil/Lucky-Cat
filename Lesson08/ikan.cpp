...
Ikan::Ikan(float sw, float sh, float x, float y) {
	this->sw = sw;
	this->sh = sh;
	this->xpos = x;
	this->ypos = y;
...
void Ikan::Update(float deltaTime)
{
}

void Ikan::Render()
{ 
	DrawIkanSprite(xpos, ypos);
}
...
void Ikan::BuildIkanSprite()
{
	...
	// Load, create texture 
	int width, height;
	unsigned char* image = SOIL_load_image("ikan.png", &width, &height, 0, SOIL_LOAD_RGBA);
...
void Ikan::DrawIkanSprite(float x, float y) {
	glEnable(GL_BLEND);
