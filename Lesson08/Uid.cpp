...
Uid::Uid(float sw, float sh, float x, float y, Player *player) {
	this->sw = sw;
	this->sh = sh;
	this->xpos = x;
	this->ypos = y;
...
void Uid::Update(float deltaTime)
{
}
...
{
	Draw();
	text->RenderText("X " + to_string((int) player->lifePoint), (frame_width + xpos + 10), 10, 0.5f, vec3(0.0f / 255.0f, 0.0f / 255.0f, 0.0f / 255.0f));

	text->RenderText("| Point : " + to_string((int) player->point), (frame_width + xpos + 53), 10, 0.5f, vec3(0.0f / 255.0f, 0.0f / 255.0f, 0.0f / 255.0f));
}


void Uid::Build()
{
	this->program = BuildShader("crateSprite.vert", "crateSprite.frag");
	...
	// Load, create texture 
	int width, height;
	unsigned char* image = SOIL_load_image("hp.png", &width, &height, 0, SOIL_LOAD_RGBA);
