...
Ground::Ground(float sw, float sh, float x, float y) {
	this->sw = sw;
	this->sh = sh;
	this->xpos = x;
	this->ypos = y;
...
void Ground::Render()
{
	DrawGroundSprite(xpos, ypos);
}

void Ground::BuildGroundSprite()
{
	...
	// Load, create texture 
	int width, height;
	unsigned char* image = SOIL_load_image("ground.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0); // Unbind texture when done, so we won't accidentily mess up our texture.

	// Set up vertex data (and buffer(s)) and attribute pointers
	frame_width = (float)sw;
	frame_height = (float)height / 4;
	GLfloat vertices[] = {
...
void Ground::DrawGroundSprite(float x, float y) {
